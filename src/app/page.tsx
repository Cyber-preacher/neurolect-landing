...
function Card({ title, desc, href }: { title: string; desc: string; href: string }) {
  return (
    <article className="rounded-2xl border/60 border bg-background/60 backdrop-blur-xl p-5 h-full flex flex-col shadow-sm hover:shadow-md transition-shadow">
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section className="mt-12 mb-8 flex flex-col items-center gap-3 text-center">
        ...
        <div className="flex gap-3">
          <Link
            href="/investors"
            className="rounded-2xl px-3.5 py-2 text-sm font-semibold shadow-sm border bg-primary text-primary-foreground hover:opacity-90"
          >
            Investors
          </Link>
          <Link
            href="/pack/neurolect-investor-pack.pdf"
            className="rounded-2xl px-3.5 py-2 text-sm font-semibold border hover:bg-accent hover:text-accent-foreground"
          >
            Download teaser
          </Link>
        </div>
